...
class VoiceInputService {
    constructor() {
        ...
        this.commands = {
            'navigate': ['navigate to', 'go to', 'take me to', 'set destination'],
        ...
        for (const [cmd, phrases] of Object.entries(this.commands)) {
            for (const phrase of phrases) {
                if (lowerTranscript.includes(phrase)) {
                    ...
                    if (cmd === 'navigate') {
                        const destinationMatch = lowerTranscript.replace(phrase, '').trim();
                        if (destinationMatch) {
    ...
    getCommandsList() {
        return {
            navigate: 'Say "navigate to [destination]" to set a destination',
